...
func TestBuffer_concat(t *testing.T) {
	...
	assert.sameValue(flatZero.length, 0);
	assert.sameValue(flatOne.toString(), 'asdf');

	const check = 'asdf'.repeat(10);

	// A special case where concat used to return the first item,
	// if the length is one. This check is to make sure that we don't do that.
	assert.notSameValue(flatOne, one[0]);
	assert.sameValue(flatLong.toString(), check);
	assert.sameValue(flatLongLen.toString(), check);
	...
	assert.throwsNodeErrorWithMessage(() => {
	  Buffer.concat([Buffer.from('hello')], -2);
	}, RangeError,
	  'ERR_OUT_OF_RANGE',
	  'The value of "length" is out of range. It must be >= 0 && <= 9007199254740991. ' +
			   'Received -2');

	const random10 = Buffer.alloc(10, 1);
